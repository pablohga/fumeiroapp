...
const calculateSubtotal = item => {
  const subtotal = convertToBRL(item.base_price * item.quantity)
...
function Cart ({ navigation }) {
  ...
  function handleOrder () {
    if (items.length) {
      ...
      navigation.navigate('Order', {entrega})
    } else {
  ...
  function renderItem ({ item }) {
    return (
      <OrderItem>
        ...
        <OrderInfo>
          <ProductTitle numberOfLines={1}>{item.name}</ProductTitle>
  ...
  return (
    <Container>
      ...
      <OrderButton onPress={() => navigation.navigate('Sizes')}>
        <OrderButtonText>Trocar Método de Entrega</OrderButtonText>
      </OrderButton>
      <Footer>
